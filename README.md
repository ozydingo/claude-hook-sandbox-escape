...
As documented, Claude Code hooks run **outside** the sandbox with full host privileges. But Claude can edit scripts inside the sandbox, often without any user interaction. Therefore, directing claude to modify a script referenced in a hook is a prime target for executing arbitrary code and connecting to networks regardless of sandbox settings.
