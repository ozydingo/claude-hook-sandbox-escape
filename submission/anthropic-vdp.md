...
However, since hook scripts run outside the sandbox, a compromised agent can find and modify a script referenced by an existing hook for arbitrary unsandboxed code execution. In the same conversation turn, the agent can trigger the hook (e.g. pre-tool-use with Bash using simply `ls`) and revert the hook to evade detection.
